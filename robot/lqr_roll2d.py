...
phi_init = 0.0 #0.75  # 初始小侧倾
move_start = 2.0  # 开始向 y 方向运动的时间
vy_cmd = 2.5  # 目标 y 方向速度
vy_ramp_time = 0  # 速度爬坡时间
move_end = 6.0  # 结束向 y 方向运动的时间 
...
sid_left = None  # 左转向执行器 id
sid_right = None  # 右转向执行器 id
...
# 舵角PID控制记录
steer_l_pos_log = []  # 左舵角位置记录
steer_r_pos_log = []  # 右舵角位置记录
steer_l_vel_log = []  # 左舵角角速度记录
steer_r_vel_log = []  # 右舵角角速度记录
steer_l_tau_log = []  # 左舵角扭矩记录
steer_r_tau_log = []  # 右舵角扭矩记录
...
class DualLoopPIDController:
    ...
    def compute(self, pos_ref, pos_meas, vel_meas, dt):
        ...
        return np.clip(torque, self.out_min, self.out_max)
...
def LPV_controller(model_in, data_in):  # 控制回调
    ...
    # 判断是否倒车：roll角超过60度视为倒车
    if abs(x[2]) > np.radians(60):
        print(f"[t={data_in.time:.2f}] 车已倒！roll = {np.degrees(x[2]):.1f}°，结束仿真")
        data_in.time = simend  # 直接跳到仿真结束时间
        return
    ...
    data_in.ctrl[sid_left] = steer_pid_left.compute(steer_ref, steer_l_pos, steer_l_vel, dt)  # 左舵角扭矩
    data_in.ctrl[sid_right] = steer_pid_right.compute(steer_ref, steer_r_pos, steer_r_vel, dt)  # 右舵角扭矩
    ...
    # 记录舵角PID控制数据
    steer_l_pos_log.append(steer_l_pos)
    steer_r_pos_log.append(steer_r_pos)
    steer_l_vel_log.append(steer_l_vel)
    steer_r_vel_log.append(steer_r_vel)
    steer_l_tau_log.append(data_in.ctrl[sid_left])
    steer_r_tau_log.append(data_in.ctrl[sid_right])
...
def controller(model_in, data_in):  # 控制回调
    ...
    # 打印调试信息（每0.5秒打印一次）
    if len(t_log) % 500 == 0:
        print(f"[t={data_in.time:.2f}] steer_l_pos={np.degrees(steer_l_pos):.2f} deg, steer_r_pos={np.degrees(steer_r_pos):.2f} deg")
...
def main():  # 主函数
    ...
    global aid_left, aid_right, sid_left, sid_right  # 执行器 id
    global adr_qpos_steer_left, adr_qvel_steer_left, adr_qpos_steer_right, adr_qvel_steer_right  # 舵角关节索引
    ...
    sid_left = get_actuator_id("left_steer_tau")  # 左舵角执行器（扭矩控制）
    sid_right = get_actuator_id("right_steer_tau")  # 右舵角执行器（扭矩控制）
    cam = mj.MjvCamera()  # 相机
    opt = mj.MjvOption()  # 选项
    ...
    while not glfw.window_should_close(window):  # 主循环
        ...
        if data.time >= simend:  # 仿真结束，退出循环
            ...
            break
        if bid_chassis >= 0:  # 如果车身存在
            cam.lookat[:] = data.xpos[bid_chassis]  # 相机跟随车身
    ...
    if len(t_log) > 1:  # 有数据才绘图
        ...
        # 舵角PID控制曲线
        ax11 = fig.add_subplot(4, 3, 12)  # 子图11
        ax11.plot(t_log, np.degrees(steer_l_pos_log), label="steer_l")  # 左舵角位置
        ax11.plot(t_log, np.degrees(steer_r_pos_log), label="steer_r")  # 右舵角位置
        ax11.set_ylabel("steer angle (deg)")  # y 标签
        ax11.set_xlabel(label_time)  # x 标签
        ax11.grid(True)  # 网格
        ax11.legend()  # 图例
        ...
        np.savez(out_data, t=np.array(t_log), x=np.array(x_log), v=np.array(v_log), 
                 ...
                 steer_l_pos=np.array(steer_l_pos_log), steer_r_pos=np.array(steer_r_pos_log),
                 steer_l_vel=np.array(steer_l_vel_log), steer_r_vel=np.array(steer_r_vel_log),
                 steer_l_tau=np.array(steer_l_tau_log), steer_r_tau=np.array(steer_r_tau_log))
